...
**优先方式（插件工具）**：调用 `mr_review_six` 工具执行六维并行审查，传参 `repoDir`、`baseSha`、`headSha`、`changedPaths`（可选）、`sourceBranch`/`targetBranch`、`mrIid`（可选）。工具内部自动完成：
- **子代理模型自动分级**：按审查类别分级（security/bug/race 等复杂类不降级，maintainability 简单类降一档；父代理已是档位表最低档时全部子代理同档；下限不低于档位表最低档，默认 = deepseek-v4-flash）。档位表可通过插件配置 `reviewLadder` 调整；
- 并行创建六个只读审查子代理（prompt 内联类别边界与发现契约）；
- 失败类别自动用上一档模型重试一次（质量护栏）；重试仍失败则标记该维度不完整。

**工具不可用时的回退方式**：并行生成六个独立的只读审查 Subagent，每个类别一个：安全、代码质量、缺陷、竞态、测试稳定性、可维护性。使用参考文档中的公共上下文与类别专属 prompt；子代理模型按上述分级规则选择（复杂类与父代理同模型，maintainability 可用低一档模型）。

统一聚合规则（两种方式共用）：
1. 等待六个终态结果全部返回。某 agent 报错、超时或触达模型容量时，用上一档模型重试该类别一次。重试仍失败则标记审查不完整，不要自动修复或声称审查干净。
2. 要求每个发现都包含类别、严重级、置信度、绝对文件路径、行号、触发条件、证据、影响与建议。干净的类别要求显式 `no findings` 结果。
3. 对同根因发现去重。保留最高合理严重级，保留跨类别标签，把推测性发现降级而非当作确认缺陷。
4. 报告顺序：先按严重级排列发现，然后干净类别、测试状态、不完整维度与残余风险。报告只留在对话中。
